...
ASSET_COLL_ID = 'projects/openet/assets/reference_et/nldas/hourly'
...
def nldas_hourly_asset(tgt_dt, overwrite_flag=False):
    """Generate hourly NLDAS asset for a single date
    ...
    """
    tgt_date = tgt_dt.strftime('%Y-%m-%dT%H:00')
    logging.info(f'Export NLDAS hourly reference ET asset - {tgt_date}')

    export_name = f'nldas_hourly_{tgt_dt.strftime(ASSET_DT_FMT)}'
    ...
    start_date = ee.Date.parse('yyyyMMddHH', tgt_dt.strftime('%Y%m%d%H'))
    end_date = start_date.advance(1, 'hour')
    # logging.info(start_date.format().getInfo())
    # logging.info(end_date.format().getInfo())

    src_coll = ee.ImageCollection(SOURCE_COLL_ID).filterDate(start_date, end_date)
    src_crs = 'EPSG:4326'
    src_size_str = '464x224'
    src_geo = [0.125, 0, -125, 0, -0.125, 53]
    src_geo_str = '[' + ', '.join(map(str, src_geo)) + ']'
    ...
    if src_count == 0:
        ...
        return f'{export_name} - No source data for hour, skipping\n'
    ...
    refet_obj = openet.refetgee.Hourly.nldas(src_coll.first())

    output_image = ee.Image([refet_obj.eto, refet_obj.etr]) \
        .rename(['eto_asce', 'etr_asce']) \
        .set({
            'system:index': tgt_dt.strftime('%Y%m%d%H'),
            'system:time_start': start_date.millis(),
            # 'system:time_end': end_date.millis(),
            # 'datetime': tgt_dt.strftime('%Y-%m-%dT%H:00'),
...
#
#     return Response(nldas_hourly_asset(tgt_dt), mimetype='text/plain')


def nldas_hourly_asset_dates(start_dt, end_dt, overwrite_flag=False):
    """Identify dates of missing NLDAS hourly assets
    ...
    """
    logging.info('\nBuilding NLDAS hourly asset ingest date list')

    task_id_re = re.compile('nldas_hourly_(?P<date>\d{8})$')
    ...
    # logging.debug('\nBuilding Date List')
    tgt_dt_list = list(datetime_range(start_dt, end_dt, hours=1))
    if not tgt_dt_list:
    ...
    task_date_list = [
        datetime.datetime.strptime(m.group('date'), ASSET_DT_FMT).strftime('%Y%m%d%H')
...
def arg_parse():
    ...
    parser = argparse.ArgumentParser(
        description='Ingest NLDAS hourly assets into Earth Engine',
