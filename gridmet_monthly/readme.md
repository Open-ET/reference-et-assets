...
```
gcloud functions deploy gridmet-reference-et-monthly --project openet --runtime python37 --entry-point cron_scheduler --trigger-http --allow-unauthenticated --memory 512 --timeout 240 --service-account="openet-assets-queue@openet.iam.gserviceaccount.com" --max-instances 1
```
...
```
gcloud scheduler jobs update http gridmet-reference-et-monthly --schedule "0 6 5,15,25 * *" --uri "https://us-central1-openet.cloudfunctions.net/gridmet-reference-et-monthly" --description "Update Monthly Bias Corrected GRIDMET Reference ET" --http-method POST --time-zone "UTC" --project openet --location us-central1 --max-retry-attempts 3 --attempt-deadline 300s --min-backoff=20s
```
